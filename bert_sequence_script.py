...
from bert_sequence import DataGenerator
...
best_file = 'best_bert_superpos.h5'
current_file = 'current_bert_superpos.h5'
...
# input layer is the BERT output layer
...
X = Bidirectional(LSTM(lstm_cells, recurrent_dropout=0.2, kernel_constraint=max_norm(mxn), return_sequences=True))(dropout)
...
Pos1 = TimeDistributed(Dropout(drop))(Pos1)
...
Pos2 = TimeDistributed(Dropout(drop))(Pos2)
...
X = Bidirectional(LSTM(lstm_cells, recurrent_dropout=0.2, kernel_constraint=max_norm(4.), return_sequences=True))(X)
X = BatchNormalization()(X)
X = Dropout(drop)(X)
...
super_output = TimeDistributed(Dense(numSuperClasses, name='super_output', activation='softmax'))(X)
...
log = CSVLogger('bert_training_log.csv')
