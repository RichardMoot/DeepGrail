...
# variant of avg_sequence_script with extra Dense layer
# this should give the model the opportunity to better exploit the ELMo
# vectors
...
#dense = TimeDistributed(Dense(256))(dropout)

# first bi-directional LSTM layer 

X = Bidirectional(LSTM(256, recurrent_dropout=0.2, kernel_constraint=max_norm(4.), return_sequences=True))(dropout)
...
X = Bidirectional(LSTM(256, recurrent_dropout=0.25, kernel_constraint=max_norm(4.), return_sequences=True))(X)
X = BatchNormalization()(X)
X = Dropout(0.25)(X)
...
X = TimeDistributed(Dense(64,kernel_constraint=max_norm(5.)))(X)
...
checkpoint = ModelCheckpoint(best_file, monitor='val_time_distributed_8_accuracy', verbose=1, save_best_only=True, save_weights_only=False, mode='max')
