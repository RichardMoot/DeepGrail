...
X_train, X_testdev, Y_train, Y_testdev = train_test_split(X, Y1, test_size=0.4)
X_test, X_dev, Y_test, Y_dev = train_test_split(X_testdev, Y_testdev, test_size=0.5)# X_train = X_train[:2048]
numClasses = len(partsofspeech1)+1
print("done")
...
Y_train_indices = lists_to_indices(Y_train, pos1_to_index, maxLen)
...
Y_dev_indices = lists_to_indices(Y_dev, pos1_to_index, max_len = maxLen)
Y_dev_oh = to_categorical(Y_dev_indices, num_classes = numClasses)

print("done")

best_file  = "best_pos1.h5"
...
#model.save('pos.h5')
get_pos_output = K.function([model.layers[7].input, K.learning_phase()], model.layers[11].output)
