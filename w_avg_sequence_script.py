...
X = Bidirectional(LSTM(128, recurrent_dropout=0.2, kernel_constraint=max_norm(6.), return_sequences=True))(dropout)
X = BatchNormalization()(X)
X = Dropout(0.2)(X)

# Pos1 output

Pos1 = TimeDistributed(Dense(32,kernel_constraint=max_norm(6.)))(X)
...
Pos2 = TimeDistributed(Dense(32,kernel_constraint=max_norm(6.)))(X)
...
X = Bidirectional(LSTM(128, recurrent_dropout=0.2, kernel_constraint=max_norm(6.), return_sequences=True))(X)
X = BatchNormalization()(X)
X = Dropout(0.2)(X)
...
X = TimeDistributed(Dense(32,kernel_constraint=max_norm(6.)))(X)
...
save_current = ModelCheckpoint(current_file, monitor='val_time_distributed_8_accuracy', verbose=1, save_best_only=False, save_weights_only=False, mode='auto', save_freq=1)
